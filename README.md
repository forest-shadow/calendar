...
### TODOS
- [x] (опционально) добавить уровни логирования в конфиг
- [x] (опционально) добавить валидацию входных данных для поля notify_before
- [x] добавить ручку для получения списка событий на день/неделю/месяц
    * Список (дата начала, дата окончания) - считаем, что периоды больше месяца не выбираются и кол-во событий мало;
- [x] настроить уведомление за N дней до события
...
- [x] Планировщик - это фоновый процесс, который не взаимодействует с пользователем и выполняет периодические задания:
    - очистка старых (более 1 года назад) событий.

### Вопросы для ревьюера:
1. **Q:** Меня несколько смутил подход, когда `repository interface` реализовывает как сервис, так и репозиторий. Это скорее частный случай или прям какой-то паттерн? Когда с этим столкнулся, прям подвис на какое то время.

    **A:** Если вкратце: сервис как структура содержит репо как встраиваемое поле с типом интерфейса репо. Это позволяет "подсаживать" другое репо сверху при инициализации сервиса (clean architecture, think abt it). Но ни о каком встраивании интерфейсов сервиса и репо между собой не может быть и речи. Это просто незачем делать.
    
    * **Q**: со временем `repository interface` стал расходиться с сервисом, который его до этого реализовывал. Стоит ли оставить общие методы в рамках одного существующего интерфейса, а расхождения добивать дополнительными интерфесами отдельно для репозитория и сервиса? Вроде в рамках переиспользования кода это норм, но не уверен что это не будет приводить к сложности восприятия такого кода. Хотя, звучит как подход написания тонких интерфейсов, которые не будут зависеть от конкретной реализации. Тут у меня некоторая дилемма: с одной стороны смущает, что сервис реализует интерфейс репозитория, а с другой стороны это позволяет не дублировать методы в двух интерфейсах (удобнинька). Можно ваше мнение?
    
        **A**: Моё мнение: сервис никогда и не при каких обстоятельствах не должен реализовывать интерфейс репозитория. Даже если с виду там одни и те же CRUD методы что и в репо.


    * **Q**: UPD: решил для себя, что можно из порождающей инстанс функции возвращаемый тип указывать в виде интерфейса. Типа так удобнее будет заменить конкретную реализацию.
    
        **A**: Вот тут я не соглашусь. Обычно удобнее в качестве возвращаемого типа указывать конкретную структуру, а не интерфейс, а в параметрах наоборот. Уже не помню конкретный кейс, но вот недавно на работе было что я в нескольких пакетах менял, кажется из-за проблем с тестированием.

        Поскольку ты сущности инжектишь "сверху", то для принимающей функции разницы не будет, лишь бы структура реализовывала все методы интерфейса.
2. **Q:** Сервис `events` вышел тупеньким. Как я понял, эта точка в основном используется когда в рамках метода одного сервиска происходит взаимодействие с другими сервисами. Так ли это, мысли на этот счет?
3. **Q:** Как правильно хендлить ошибки в фоновых задачах? Я вынес это в апп, где эти джобы и стартовал. Оцени как я реализовал это в контексте сервиса `events`. Ошибки, предложения? Как будто можно было бы еще разделить на абстракции, но я решил пока не усложнять.
4. **Q:** Оцени как реализован метод `GetRecentEvents` в репозитории `events`. Правильно ли что я заюзал CTE там?
5. **Q:** Часто ли используется цветной вывод логов в stdout? Например, для каких-то отдельных сервисов или джобов. Если да, как это правильно реализовать? Я пробовал добавлять escape-последовательности в логгер, но это приводило к тому что конфиги логгера перестали корректно синхронизироваться и иногда выдавали кашу. Если сталкивался с таким, подскажи как это побеждается?
    * UPD: дело видимо даже не в цветах. И без них сейчас в кашу выдает порой. Видимо в чем то другом проблема.
6. **Q:** Тесты. Подскажи, для чего их можно написать? Какие тулзы и подходы лучше использовать?